...
export default function DeckManager({ open, onOpenChange, loadRoom, importRoom }: { open: boolean; onOpenChange: (v: boolean) => void; loadRoom?: () => Promise<unknown>; importRoom?: (d: Deck) => Promise<boolean> }) {
  ...
  return <Dialog open={open} onOpenChange={onOpenChange}><DialogContent className="modal deck-modal deck-builder-modal">
    ...
    <div className="deck-builder"><section className="deck-catalog-panel">
      ...
      <div className="builder-card-grid">{catalog.map((card) => <article key={card.id} className="builder-card"><button className="builder-card-image" onClick={() => setSelected(card)} aria-label={`Ver ${card.name}`}><img src={card.image} alt={`Carta ${card.name}`} loading="lazy" /></button><div><strong>{card.name}</strong><small>{card.type} · {card.cost}{card.type === 'Aliado' ? ` · ${card.strength} fuerza` : ''}</small></div><button className="builder-add" onClick={() => add(card)} aria-label={`Añadir ${card.name}`}>＋ <span>{copies(card)}</span></button></article>)}</div>
