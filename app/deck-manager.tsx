...
export default function DeckManager({ open, onOpenChange, loadRoom, importRoom }: { open: boolean; onOpenChange: (v: boolean) => void; loadRoom?: () => Promise<unknown>; importRoom?: (d: Deck) => Promise<boolean> }) {
  ...
  const editions = useMemo(() => ['Todas', ...Array.from(new Set(catalog.map((card) => card.edition))).sort()], [catalog]);
  const races = useMemo(() => ['Todas', ...Array.from(new Set(catalog.map((card) => card.race).filter(Boolean))).sort()], [catalog]);
  const filtered = useMemo(() => { const needle = query.trim().toLocaleLowerCase('es'); return catalog.filter((card) => (!needle || `${card.id} ${card.name} ${card.effect}`.toLocaleLowerCase('es').includes(needle)) && (edition === 'Todas' || card.edition === edition) && (type === 'Todas' || card.type === type) && (race === 'Todas' || card.race === race) && (cost === 'Todos' || card.cost === Number(cost))).slice(0, 160); }, [catalog, query, edition, type, race, cost]);
