...
export function expire(room) {
  for (const p of room.players)
    for (const c of p.cards) {
      if (c.transformation && expired(c.transformation, room)) {
        Object.assign(c, c.transformation.form);
...
export function effectAction(room, p, a, h) {
  ...
  let effectOrigin;
  const effectActor = h.effectActorId || p.id;
  if (a.effectCardId) {
    const owner = room.players.find((x) => x.cards.some((c) => c.id === a.effectCardId));
    const origin = owner?.cards.find((c) => c.id === a.effectCardId);
    const inspection = room.inspections[effectActor];
    const visible = origin && (
      origin.revealed ||
      !['mano', 'castillo'].includes(origin.zone) ||
      (origin.zone === 'mano' && owner.id === effectActor) ||
      (inspection?.owner === owner.id && inspection.zone === origin.zone && inspection.ids.includes(origin.id))
    );
    if (!visible) fail('La carta que origina el efecto ya no está visible. Vuelve a seleccionarla');
    reason = text(origin.name);
    effectOrigin = { cardId: origin.id, playerId: owner.id, actorId: effectActor };
  }
  ...
  if (a.type === 'look') {
    ...
    message = effectAction(room, p, { ...req.payload, type: 'effect' }, { ...h, effectActorId: req.actor });
    ...
    let copyForm;
    if (operation === 'copy') {
      const source = room.players.flatMap((x) => x.cards).find((c) => c.id === a.sourceId);
      if (cards.length !== 1) fail('Elige una sola carta que recibirá la copia');
      if (!source || !['defensa', 'ataque', 'apoyo', 'reserva', 'pagado', 'cementerio', 'destierro'].includes(source.zone))
        fail('Elige una carta visible de la mesa para copiar');
      if (source.id === cards[0].id) fail('La carta no puede copiarse a sí misma');
      if (!['full', 'ability'].includes(a.copyMode)) fail('Elige qué quieres copiar');
      copyForm = definition(source);
    }
    if (operation === 'transform') {
      ...
      if (
        a.traits &&
        (!Array.isArray(a.traits) || a.traits.some((x) => !STATUS.includes(x)))
      )
        fail('Estado inválido');
    }
    ...
    for (const c of cards) {
      ...
      if (operation === 'transform' || operation === 'copy') {
        c.originalForm ??= definition(c);
        c.transformation = {
          ...duration(room, p, a),
          form: c.transformation?.form || {
            ...definition(c),
            modifiers: structuredClone(c.modifiers || []),
            statuses: structuredClone(c.statuses || {}),
          },
        };

        if (operation === 'copy') {
          if (a.copyMode === 'ability') c.effect = copyForm.effect;
          else Object.assign(c, copyForm);
        } else {
          c.type = a.cardType;
          c.strength = Number(a.strength);
          c.effect = typeof a.effect === 'string' ? text(a.effect) : c.effect;
          c.modifiers = [];
          c.statuses = Object.fromEntries(
            (a.traits || []).map((x) => [x, { ...duration(room, p, a), reason }]),
          );
        }
      }
      ...
      if (operation === 'cancelAttack') {
        ...
        c.blocks = null;
        for (const battle of room.pendingBattles || []) {
          battle.rows = battle.rows.filter((row) => row.cardId !== c.id);
        }
        room.pendingBattles = (room.pendingBattles || []).filter((battle) => battle.rows.length);
      }
    ...
    message = `${operation}${operation === 'move' ? ` a ${a.zone}` : ''}: ${cards.length} carta(s)${names ? ` (${names})` : ''}. ${reason}`;
    if (operation === 'copy')
      message = `${names || 'Una carta'} copió ${a.copyMode === 'ability' ? 'la habilidad de' : 'nombre, imagen y atributos de'} ${copyForm.name}. ${reason}`;
  } else fail('Acción de efecto desconocida');

  if (effectOrigin) {
    room.lastEffect = { ...effectOrigin, action: a.type, operation: a.operation || null, turn: room.turn };
  }
  return message;
}
