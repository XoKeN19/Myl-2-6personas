...
export function definition(data = {}) {
  ...
  const remoteImage = typeof data.image === 'string' && /^https:\/\/(?:api\.myl\.cl|myths\.cl)\/(?:static\/cards\/|cards\/)/.test(data.image) && data.image.length <= 500;
  return {
    catalogId: str(data.catalogId || (!data.deckKey ? data.id : ''), 100),
    edition: str(data.edition, 140),
    name: str(data.name) || 'Carta sin nombre',
    type: TYPES.includes(data.type)
      ? data.type
      : fail('Tipo de carta inválido'),
    effect: str(data.effect, 3000),
    image: remoteImage || (typeof data.image === 'string' && /^data:image\/(jpeg|png|webp);base64,[A-Za-z0-9+/=]+$/.test(data.image) && data.image.length <= 180000) ? data.image : '',
...
export function migrate(room) {
  if (room.phase === 'Bloqueo') room.phase = 'Ataque';
  ...
  for (const p of room.players) {
    ...
    p.deckLoaded ??= p.deckList.length===50&&p.deckList.every(c=>!/^Carta \d+$/.test(c.name));
  }
  return room;
}
...
export function action(room, token, a) {
  ...
  const p = room.players.find((p) => p.token === token);
  if (!p) fail('Sesión inválida');
  if(room.initiative && Date.now()<room.initiative.endsAt)fail('Espera a que termine la tirada inicial');
  const own = () =>
  ...
  switch (a.type) {
    ...
    case 'add':
      ...
      // Crear una carta también cuenta como comenzar a armar el mazo: permite
      // que la sala muestre que este jugador ya está preparando el suyo.
      if (!room.started) p.deckLoaded = true;
      message =
        'añadió una carta' + (a.reason ? `: ${str(a.reason, 300)}` : '');
      break;
    case 'edit': {
      const c = own(),
        n = definition({ ...c, ...a.card }),
    ...
    case 'freeMulligan': {
      if (room.started || !p.ready)
        fail('Prepara la mano antes; sólo antes de comenzar');
      ...
      message =
        'hizo ' +
        (a.type === 'houseMulligan' ? 'Volver a ocho' : 'mulligan') +
        ' y quedó con ' +
        count +
        ' cartas';
      break;
    ...
    case 'strike': {
      ...
      if (
        ...
        !assignments.length ||
        new Set(assignments.map((x) => x.cardId)).size !== assignments.length
      )
        fail('Elige atacantes distintos');
      const used = room.struck || [];
      const attacks = assignments.map((x) => {
        const c = p.cards.find(
          (c) =>
            c.id === x.cardId && c.zone === 'ataque' && c.type === 'Aliado',
        );
        const target = room.players.find(
          (q) => q.id === x.target && q.id !== p.id && q.cards.some((card) => card.zone === 'castillo'),
        );
        if (!c || !target || used.includes(c.id))
          fail('Atacante o destino inválido, o daño ya aplicado este turno');
        return { c, target };
      });
      room.pendingBattles ??= [];
      for (const target of new Set(attacks.map((x) => x.target.id))) {
        room.pendingBattles.push({
          id: id(),
          attacker: p.id,
          attackerName: p.name,
          target,
      ...
      for (const { c, target } of attacks) c.target = target.id;
      room.struck = [...used, ...attacks.map((x) => x.c.id)];
      room.phase = 'Guerra de Talismanes';
      message = 'declaró ataque; comienza la respuesta, bloqueo y Guerra de Talismanes';
      break;
    }
    case 'defend': {
      const battle = room.pendingBattles?.find(
        (x) => x.id === a.battleId && x.target === p.id,
      );
      if (!battle) fail('Combate no disponible');
      const attacker = room.players.find((x) => x.id === battle.attacker);
      if (
        !Array.isArray(a.rows) ||
        a.rows.length !== battle.rows.length ||
        new Set(a.rows.map((x) => x.cardId)).size !== a.rows.length
      )
        fail('Revisa todos los atacantes');
      const blockers = new Set();
      const damage = a.rows.reduce((sum, row) => {
        if (!battle.rows.some((x) => x.cardId === row.cardId))
          fail('Atacante inválido');
        if (row.blocker) {
          if (
            blockers.has(row.blocker) ||
            !p.cards.some(
              (c) =>
                c.id === row.blocker &&
                c.type === 'Aliado' &&
            ...
            fail('Bloqueador inválido o repetido');
          blockers.add(row.blocker);
        }
        const value = num(row.damage, 0, 999);
        if (
          !attacker?.cards.some(
            (c) =>
              c.id === row.cardId && c.zone === 'ataque' && c.type === 'Aliado',
          ) &&
          value > 0
        )
          fail('Ese aliado ya no está atacando');
        return sum + value;
      }, 0);
      const actual = take(p, damage, 'cementerio');
      room.combatEvents ??= [];
      ...
      room.combatEvents = room.combatEvents.slice(-30);
      room.pendingBattles = room.pendingBattles.filter((x) => x !== battle);
      room.phase = room.pendingBattles.length ? 'Guerra de Talismanes' : 'Asignación de daño';
      message =
        'resolvió el ataque de ' +
        battle.attackerName +
        ': ' +
        actual +
        ' cartas botadas (daño ajustado por el defensor)';
      break;
    }
    case 'cancelStrike': {
      const battle = room.pendingBattles?.find(
        (x) =>
          x.id === a.battleId && (x.target === p.id || x.attacker === p.id),
      );
      if (!battle) fail('Combate no disponible');
      const cardId = typeof a.cardId === 'string' ? a.cardId : '';
      if (cardId) {
        const row = battle.rows.find((x) => x.cardId === cardId);
        if (!row) fail('Ese aliado no participa en este ataque');
        battle.rows = battle.rows.filter((x) => x !== row);
        const attacker = room.players.find((x) => x.id === battle.attacker);
        const ally = attacker?.cards.find((x) => x.id === cardId);
        if (ally) { ally.target = null; ally.blocks = null; }
        if (!battle.rows.length) room.pendingBattles = room.pendingBattles.filter((x) => x !== battle);
        message = 'canceló solamente el ataque de ' + row.name;
      } else {
        for (const row of battle.rows) {
          const attacker = room.players.find((x) => x.id === battle.attacker);
          const ally = attacker?.cards.find((x) => x.id === row.cardId);
          if (ally) { ally.target = null; ally.blocks = null; }
        }
        room.pendingBattles = room.pendingBattles.filter((x) => x !== battle);
        message = 'canceló todo el daño de ese grupo de ataque';
      }
      break;
    }
    case 'groupGold': {
      const gold = p.cards.filter(
        (c) => c.type === 'Oro' && c.zone === 'pagado',
      );
      for (const c of gold) move(p, c, 'reserva');
      message = 'agrupó ' + gold.length + ' oros en Reserva';
      break;
    }
    case 'revealUntil': {
      const castle = p.cards.filter((c) => c.zone === 'castillo');
      const targetType = TYPES.includes(a.cardType) ? a.cardType : 'Aliado';
      const found = castle.findIndex((c) => c.type === targetType);
      const shown = castle.slice(0, found < 0 ? castle.length : found + 1);
      room.revealEvent = {
        id: id(),
        player: p.id,
        name: p.name,
        cards: shown.map((c) => ({ id: c.id, ...definition(c) })),
        found: found >= 0,
        targetType,
      };
      message =
        'mostró ' +
        shown.length +
        ' cartas hasta encontrar ' + targetType +
        (found < 0 ? ' (no hay cartas de ese tipo)' : '');
      break;
    }
    case 'start':
      ...
      if (
        ...
        room.players.some((q) => !q.ready && !q.deckLoaded)
      )
        fail('Deben entrar todos los jugadores de la sala y cargar sus mazos');
      for(const q of room.players){if(!q.ready){shuffle(q);take(q,Math.max(0,8-q.cards.filter(c=>c.zone==='mano').length),'mano');q.ready=true;}}
      room.initiative = rollInitiative(room.players);
      room.active = room.initiative.winner;
      room.started = true;
      ...
      break;
    case 'phase': {
      const selectedPhase = a.phase === 'Bloqueo' ? 'Ataque' : a.phase;
      if (!PHASES.includes(selectedPhase)) fail('Fase inválida');
      room.phase = selectedPhase;
      room.battleReady = [];
      message = `cambió libremente la fase a ${selectedPhase}`;
      break;
    }
    case 'next':
      active();
      if (room.pendingBattles?.length)
        fail(
          'Resuelvan o cancelen los ataques pendientes antes de pasar turno',
        );
      const automaticDraw =
        room.phase === 'Asignación de daño' &&
        !room.drawn &&
        room.turn !== 1 &&
        p.cards.some((c) => c.zone === 'castillo')
          ? take(p, 1, 'mano')
          : 0;
      if (automaticDraw) room.drawn = true;
      {
        const currentIndex = room.players.findIndex((q) => q.id === p.id);
        const nextPlayer = Array.from({ length: room.players.length }, (_, offset) =>
          room.players[(currentIndex + offset + 1) % room.players.length],
        ).find((q) => q.cards.some((c) => c.zone === 'castillo'));
        if (!nextPlayer) fail('Todos los Castillos están vacíos. La partida terminó');
        room.active = nextPlayer.id;
      }
      room.turn++;
      ...
      message =
        'pasó el turno; se agruparon las cartas y los oros pagados volvieron a reserva' +
        (automaticDraw ? '. Robó automáticamente una carta tras la asignación de daño' : '');
      break;
    case 'timer': {
      const now = Date.now();
  ...
  room.defeated = room.players
    .filter(
      (x) =>
        (x.ready || room.started) &&
        !x.cards.some((c) => c.zone === 'castillo'),
    )
