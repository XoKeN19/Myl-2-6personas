...
export function action(room, token, a) {
  ...
  switch (a.type) {
    ...
    case 'strike': {
      ...
      for (const { c, target } of attacks)
        groups.set(target.id, (groups.get(target.id) || 0) + power(c));
      room.combatEvents ??= [];
      for (const [targetId, strength] of groups) {
        const target = room.players.find((q) => q.id === targetId);
        const actual = take(target, strength, 'cementerio');
      ...
      message =
        ...
        [...groups]
          .map(
            ([target, n]) =>
              room.players.find((q) => q.id === target).name + ' ' + n,
          )
          .join(', ');
      break;
    }
